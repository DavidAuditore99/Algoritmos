...
/*
//DFT ES
//Sum n=0 to N (X(n)e^(-j2pi)/N*kn)
    ó
                (Xn[Cos((2pi/N)*kn)-jsin((2pi/N)*kn)])
    n: es el elemento del arreglo
    N:numero de datos
    k:Numero de elemento del nuevo arreglo
*/
...
int main(){
    ...
    DFT(N,X,Xre,Xim);
    printf("_________________RESULTADOS____________________\n");
    for (i = 0; i <N; i++)
    {
        if (Xim[i]<0)
        {
